...
public class CaptureCameraPreview extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        ...
        int cam_width = settings.getInt("WIDTH", -1);
        int cam_height = settings.getInt("HEIGHT", -1);
        if(cam_width == -1 || cam_height == -1) {
            List<Camera.Size> previewSizes = parameters.getSupportedPreviewSizes();
            Camera.Size size = getOptimalSize(previewSizes, w, h);
            cam_width = size.width;
            cam_height = size.height;
            SharedPreferences.Editor editor = settings.edit();
            editor.putInt("WIDTH", cam_width);
            editor.putInt("HEIGHT", cam_height);
            editor.commit();
        }
